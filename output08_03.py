...
def get_common_news(news_obj):
    ...
    rel_links = news_obj.select('.relate_new .item_relate')

    for rel_link in rel_links:
        rel_link_title = rel_link.select('a')[0].text
        rel_link_url = rel_link.select('a')[0].get('herf')
...
def get_html_from_daum_news(category):
    ...
    return req.requst('GET', surl).data
...
def parse_side_news(bsboj):
    s_news_tag_list = bsboj.select('.list_mainnews2 li')
    side_news_list = []
